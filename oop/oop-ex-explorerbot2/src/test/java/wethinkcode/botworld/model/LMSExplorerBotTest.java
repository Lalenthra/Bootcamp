...
import static wethinkcode.botworld.model.ExplorerBot.WORLD_MAX_X;
import static wethinkcode.botworld.model.ExplorerBot.WORLD_MAX_Y;
import static wethinkcode.botworld.model.Heading.*;

public class LMSExplorerBotTest
{
    static final int X_INSIDE = WORLD_MAX_X / 2;
    static final int Y_INSIDE = WORLD_MAX_Y / 2;
    ...
    void createBot_outsideWorld_xTooLarge(){
        assertThrows( IllegalArgumentException.class, () -> new ExplorerBot( WORLD_MAX_X + 1, 0 ));
    }
    ...
    void createBot_outsideWorld_yTooLarge(){
        assertThrows( IllegalArgumentException.class, () -> new ExplorerBot( X_INSIDE, WORLD_MAX_Y + 1 ));
    }

    @Test
    void createBot_insideWorld(){
        new ExplorerBot( X_INSIDE, Y_INSIDE );
    }
    ...
    void move_south_outsideWorld(){
        ExplorerBot bot =  new ExplorerBot( X_INSIDE, WORLD_MAX_Y );
        bot.turnTo( S );
        bot.move();
        assertEquals( WORLD_MAX_Y, bot.position().y() );
    }
    ...
    void move_east_outsideWorld(){
        ExplorerBot bot =  new ExplorerBot( WORLD_MAX_X, Y_INSIDE );
        bot.turnTo( E );
        bot.move();
        assertEquals( WORLD_MAX_X, bot.position().x() );
    }
}